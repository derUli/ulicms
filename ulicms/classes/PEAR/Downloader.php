...
class PEAR_Downloader extends PEAR_Common {
				...
				if (count ( $deps )) {
					...
					if (isset ( $deps ['required'] ) && is_array ( $deps ['required'] )) {
						foreach ( $deps ['required'] as $type => $dep ) {
							...
							if (! isset ( $dep [0] )) {
								if (PEAR::isError ( $e = $depchecker->{
                                            "validate{$type}Dependency"} ( $dep, true, $params ) )) {
									$failed = true;
									if (! isset ( $this->_options ['soft'] )) {
								...
								foreach ( $dep as $d ) {
									if (PEAR::isError ( $e = $depchecker->{
                                                "validate{$type}Dependency"} ( $d, true, $params ) )) {
										$failed = true;
										if (! isset ( $this->_options ['soft'] )) {
											$this->log ( 0, $e->getMessage () );
										}
						...
						if (isset ( $deps ['optional'] ) && is_array ( $deps ['optional'] )) {
							foreach ( $deps ['optional'] as $type => $dep ) {
								if (! isset ( $dep [0] )) {
									if (PEAR::isError ( $e = $depchecker->{
                                                "validate{$type}Dependency"} ( $dep, false, $params ) )) {
										$failed = true;
										if (! isset ( $this->_options ['soft'] )) {
											$this->log ( 0, $e->getMessage () );
										}
									...
									foreach ( $dep as $d ) {
										if (PEAR::isError ( $e = $depchecker->{
                                                    "validate{$type}Dependency"} ( $d, false, $params ) )) {
											$failed = true;
											if (! isset ( $this->_options ['soft'] )) {
												$this->log ( 0, $e->getMessage () );
											}
	 ...
	 *
	 * @param array $
	 *        	output of {@link parsePackageName()}
	 * @access private
	 */
	 ...
	 *
	 * @param array $
	 *        	an array of downloaded PEAR_Downloader_Packages
	 ...
	 *        	updates
	 * @param false $
	 *        	|string|array $lastmodified header values to check against for caching
	 *        	use false to return the header values from this download
	 * @param false $
	 *        	|array $accept Accept headers to send
	 * @param false $
	 *        	|string $channel Channel to use for retrieving authentication
