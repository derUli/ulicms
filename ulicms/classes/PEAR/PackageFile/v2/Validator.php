...
class PEAR_PackageFile_v2_Validator {
		...
		foreach ( array (
			...
			if (isset ( $cond [$type] )) {
				...
				foreach ( $iter as $package ) {
					if ($type == 'extension') {
						$this->{
                            "_validate{$type}Dep"} ( $package, false, true );
					} else {
						$this->{
                            "_validate{$type}Dep"} ( $package, true );
					}
				}
			}
		}
					...
					foreach ( array (
						...
						if (isset ( $this->_packageInfo ['dependencies'] [$simpledep] [$type] )) {
							...
							foreach ( $iter as $package ) {
								...
								$this->{
                                    "_validate{$type}Dep"} ( $package, "<$simpledep>" );
							}
						}
					}
					if ($simpledep == 'optional') {
					...
					foreach ( array (
						...
						if (isset ( $group [$type] )) {
							...
							foreach ( $iter as $package ) {
								...
								$this->{
                                    "_validate{$type}Dep"} ( $package, '<group name="' . $group ['attribs'] ['name'] . '">' );
							}
						}
					}
				}
	 ...
	 * @param boolean $
	 *        	whether to analyze $file as the file contents
	 * @return mixed
	 */
