...
class Template
{
    ...
    {
        $query = db_query("SELECT name, link_url, image_url, `type`, html FROM " . tbname("banner") . " WHERE enabled = 1 and (language IS NULL OR language='" . db_escape($_SESSION["language"]) . "') ORDER BY RAND() LIMIT 1");
