...
<?php
	...
	if (count ( $modules ) > 0) {
		...
		for($i = 0; $i < count ( $modules ); $i ++) {
			...
			$module_has_admin_page = ((faster_file_exists ( getModuleAdminFilePath ( $modules [$i] ) ) or faster_file_exists ( getModuleAdminFilePath2 ( $modules [$i] ) ) or ($controller and method_exists ( $controller, "settings" ))) && ! in_array ( $modules [$i], $disabledModules ));
