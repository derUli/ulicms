...
(function($) {
	...
	var setShiftEvent = function() {
		...
		$("html").bind('keydown', shiftHandler).bind('keyup', shiftHandler);
	};
	...
	var getElemIndex = function(rangeInfo, element) {
		...
		if (rangeInfo.cache && idx !== undefined
				&& $element.data('_sc_cache_version') === cacheVersion) {
			return idx;
		}
		var length = rangeInfo.$elements.length;
		for (var i = 0; i < length; i++) {
			$($elements[i]).data('_sc_idx', i).data('_sc_cache_version',
					cacheVersion);
	...
	var onChange = function(event) {
		...
		var fromIdx = getElemIndex(rangeInfo, startElement), toIdx = getElemIndex(
				rangeInfo, this), lastIdx = getElemIndex(rangeInfo, lastElement);
		...
		for (var j = lowerIdx; j <= upperIdx; j++) {
			if ($elements[j] !== this) {
				rangeInfo.click.call($elements[j], rangeInfo.status
						.call(startElement));
		...
		if (useLoop) {
			for (var x = lowerIdx; x <= upperIdx; x++) {
				if ($elements[x] !== this) {
					rangeInfo.click.call($elements[x], !rangeInfo.status
							.call(startElement));
	...
	$.fn.shiftClick = function(userOpts) {
		...
		setShiftEvent();
		$(opts.delegate).delegate(this.selector, 'change', rangeInfo, onChange);
		return this;
	};
...
function showChangedOnlineStatus(name, online) {
	...
	if (online) {
		$("#message").html(
				"<span style='color:green'>" + name + " ist online</span>");
	} else {
		$("#message").html(
				"<span style='color:red'>" + name + " ist offline</span>");
	}
