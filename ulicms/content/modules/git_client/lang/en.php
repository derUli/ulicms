<?php

add_translation("git_client_headline", "Git Client [%branch%]");
add_translation("commit", "Commit");
add_translation("message", "Nachricht");
add_translation("git_pull", "Pull");
add_translation("latest_commit_is_now", "Der neuste Commit ist jetzt");
add_translation("there_are_local_changes", "Es sind lokale Änderungen vorhanden.");
add_translation("no_changes", "Keine Änderungen vorhanden.");
